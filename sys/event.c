    ...
    __in PEVENT_CONTEXT EventContext)
{
    ...
    DOKAN_LOG_FINE_IRP(RequestContext, "Pending Registration: %s",
        DokanGetNTSTATUSStr(status));
    return status;
}
...
DokanCompleteIrp(__in PREQUEST_CONTEXT RequestContext)
{
        ...
        thisEntry = nextEntry)
    {
        ...
        if (!RequestContext->Dcb->AllowIpcBatching)
        {
            ...
            {
                result =
                    DokanLogError(&logger, STATUS_INVALID_PARAMETER,
...
BOOLEAN HasSameOwner(__in PDOKAN_LOGGER Logger, __in PDEVICE_OBJECT NewDevice,
...
DokanEventStart(__in PREQUEST_CONTEXT RequestContext)
{
    ...
    case DOKAN_NETWORK_FILE_SYSTEM:
        ...
        break;
    default:
        DOKAN_LOG_FINE_IRP(RequestContext, "Unknown device type: %d",
    ...
    // 盘符被占用且使用mountmanager，尝试释放旧盘符或分配新盘符
